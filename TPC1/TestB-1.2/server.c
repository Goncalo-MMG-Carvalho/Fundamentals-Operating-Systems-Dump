...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        printf("Request (file %s) handled by new process.\n", req.file_name);
        int status;
        if(waitpid(-1, &status, WNOHANG)){ // if is already dead or ended with error
            if (WIFEXITED(status))
                printf ("Process ended with status: %s.\n", strerror(WEXITSTATUS(status)));
        }
    }
}
